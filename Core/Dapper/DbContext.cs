...
namespace Core.Dapper
{
    public class DbContext : IDbContext, IUnitOfWork
    {
        ...
        public IDbTransactionHandle Begin(IsolationLevel isolationLevel = IsolationLevel.ReadCommitted)
        {
            if (disposed)
            {
        ...
        protected virtual IDbTransactionHandle EnsureTransaction(IsolationLevel isolationLevel)
        {
